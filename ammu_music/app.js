...
let userProfile = { name: 'Amarjeet Kumar', avatar: 'icon-192.png' };
...
document.getElementById('btn-view-playlist-author').onclick = () => {
  ...
  document.getElementById('author-avatar-display').src = author.avatar || 'icon-192.png';
...
// Artwork Mode Switcher (Standard, Vinyl)
...
// Granular Universal Export Engine
document.getElementById('btn-export-backup').onclick = async () => {
  ...
  showNotification('Packaging export data...');
  ...
  const exportedPlaylists = allPlaylists.filter(p => selectedPlIds.has(p.id)).map(p => ({
    id: p.id,
    name: p.name,
  ...
  for (const t of allTracks) {
    if (!selectedPlIds.has(t.playlistId) && t.playlistId !== 'all') continue;
    let audioData = null;
  ...
  const fileName = `Ammu_Music_Backup_${includeAudio ? 'FULL' : 'META'}_${Date.now()}.json`;

  // Try Native Custom Folder Directory Picker
  if ('showSaveFilePicker' in window) {
    try {
      ...
      await dbOps.addAuditLog({ type: 'EXPORT', desc: `Exported ${exportedTracks.length} track(s) to custom folder` });
  ...
  await dbOps.addAuditLog({ type: 'EXPORT', desc: `Exported ${exportedTracks.length} track(s) to Downloads/` });
  renderAuditLogs();
  showNotification(`Exported to Downloads folder: ${fileName}`);
};

// Two-Step Pre-Import Engine
...
document.getElementById('btn-confirm-final-import').onclick = async () => {
  ...
  const author = pendingImportPayload.author || { name: 'External Creator', avatar: 'icon-192.png' };

  // Save Playlists with author protection lock
  ...
  for (const trk of pendingImportPayload.tracks) {
    ...
    importedSongs.push(trk.name);
  }

  await dbOps.addAuditLog({ type: 'IMPORT', desc: `Imported ${importedSongs.length} tracks by ${author.name}` });

  document.getElementById('pre-import-modal').style.display = 'none';
  await loadPlaylists();

  // Show Rich Success Summary Window
  document.getElementById('success-author-avatar').src = author.avatar || 'icon-192.png';
  document.getElementById('success-author-label').textContent = `Curated by ${author.name}`;
  const summaryBox = document.getElementById('success-songs-summary');
  summaryBox.innerHTML = importedSongs.map((s, i) => `<div>${i + 1}. ${s}</div>`).join('');
...
document.getElementById('btn-batch-delete').onclick = async () => {
  ...
  showNotification(`Deleted ${selectedTrackIds.size} track(s)`);
  multiSelectMode = false;
  selectedTrackIds.clear();
  document.getElementById('batch-action-bar').style.display = 'none';
  loadTracks();
...
boxBtnPrev.onclick = () => { triggerHaptic(20); playTrack(currentIndex > 0 ? currentIndex - 1 : tracks.length - 1); };
...
// Touch Cover Swipe
let touchStartX = 0;
const swipeArea = document.getElementById('art-display-box');
