...
// INDEXEDDB ENGINE
// -------------------------------------------------------------
const DB_NAME = 'AmmuMusicDB_v190';
const DB_VER = 1;
let db;
...
document.getElementById('pl-menu-btn-edit').onclick = () => {
  ...
  // Check if locked and not yet unlocked on this device
...
// Edit Playlist Details Modal Logic (Supports Local-Only Customization)
...
document.getElementById('btn-save-edit-playlist').onclick = async () => {
  ...
  // Preserve original metadata for export while applying local edits
...
// Fixed User Profile Editing Logic
...
function downloadSingleTrack(trk, notify = true, explicitAuthorName = '') {
  ...
  // Exact requested format: [Song Title] [Author]'s music taste.mp3
...
async function executeUniversalExport(includeAudio, includeMarkers, includeImages, includeClips, selectedPlIds) {
  ...
  // Export always strips local edits and emits the original creator attributes
  ...
  // Exact requested format: Ammu_[FULL/META]_[Author Name]'s music taste backup.json
...
document.getElementById('btn-confirm-final-import').onclick = async () => {
  ...
  // Evaluate Creator Passkey for Permanent Device Unlock
