...
// Complete Full Profile & Library Exporter
...
document.getElementById('btn-export-backup').onclick = async () => {
  ...
  showNotification('Packaging selective backup data...');
...
function handleDirectImport(file, bypassOnboarding) {
  if (!file) return;
  const reader = new FileReader();
...
// Add Songs with Duplicate Detection
